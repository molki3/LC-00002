...
export default function PointCanvas({
  ...
  const pinchRef = useRef<{ active: boolean; startDist: number; startScale: number; cx: number; cy: number }>(
    { active: false, startDist: 0, startScale: 1, cx: 0, cy: 0 }
  )

  // tap / double-tap
  const TAP_MAX_DIST = 8      // px
  const DOUBLE_TAP_MS = 280   // ventana doble tap
  const tapRef = useRef<{ lastTime: number; x: number; y: number; active: boolean }>({
    lastTime: 0, x: 0, y: 0, active: false
  })
  ...
  // ===== doble-click (desktop) → crear punto en el fondo =====
  ...
  // ===== mouse pan =====
  const handleMouseEnter: React.MouseEventHandler<HTMLDivElement> = () => lockBodyScroll()
  ...
  const onPointerMove: React.PointerEventHandler<HTMLDivElement> = (e) => {
    ...
    if (isPanningRef.current && pointers.current.size === 1) {
      ...
      // si se movió demasiado, ya no es tap
      if (tapRef.current.active) {
        const mdx = Math.abs(p.x - tapRef.current.x)
        const mdy = Math.abs(p.y - tapRef.current.y)
        if (mdx > TAP_MAX_DIST || mdy > TAP_MAX_DIST) tapRef.current.active = false
      }
    }
  }
  ...
  const onPointerUpOrCancel: React.PointerEventHandler<HTMLDivElement> = (e) => {
    ...
    // Solo detectar doble-tap si NO fue sobre un pin
    if ((e.target as HTMLElement).dataset.pin === '1') {
      tapRef.current.active = false
      return
    }

    // Si el candidato a tap sigue activo y el desplazamiento fue pequeño…
    if (tapRef.current.active) {
      tapRef.current.active = false
      const dx = Math.abs(e.clientX - tapRef.current.x)
      const dy = Math.abs(e.clientY - tapRef.current.y)
      if (dx <= TAP_MAX_DIST && dy <= TAP_MAX_DIST) {
        const now = performance.now()
        const dt = now - tapRef.current.lastTime
        if (dt > 0 && dt <= DOUBLE_TAP_MS) {
          // 🔥 doble-tap → crear punto
          tapRef.current.lastTime = 0
          onAddPoint?.(clientToNormalized(e.clientX, e.clientY))
        } else {
          // primer tap: no hacemos nada, solo armamos ventana
        ...
        >
          ...
          {natural.w > 0 && points.map((p: any) => (
            <button
              ...
              data-pin="1"
              type="button"
              className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full bg-rose-500 shadow ring-2 ring-white cursor-pointer z-20"
              style={{
                left: (p.x ?? 0) * natural.w,
                top: (p.y ?? 0) * natural.h,
                width: 14, height: 14, outline: '2px solid rgba(0,0,0,0.35)',
              }}
              title={p.name ?? 'Punto'}
              onClick={(e) => {
                e.stopPropagation()
          ...
          <div className="pointer-events-none absolute inset-0">
            {points.map((p: any) => {
              ...
              return (
                <div
                  key={`lbl-${p.id}`}
                  className="absolute -translate-x-1/2 translate-y-3 whitespace-nowrap rounded bg-black/70 px-2 py-0.5 text-[11px] text-white shadow pointer-events-none"
